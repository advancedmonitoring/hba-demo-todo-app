...
class EventPermissions:
    ...
    @staticmethod
    def event_create_note():
        """ Каждый может создать запись """
        return True
    ...
    def _is_note_author(self, note: Note):
        return note.author_id == self.user.id

    def event_get_note(self):
        """ Получить можно только свою запись """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)

    def event_update_note(self):
        """ Обновить можно только свою запись """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)

    def event_delete_note(self):
        """ Удалить можно только свою запись """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)

    def event_create_todo(self):
        """ Каждый может создать дело, но только внутри собственных записей """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)
    ...
    def event_get_todos():
        """
        Получить список дел имеет право любой пользователь.
        Содержимое списка - задача не класса проверки прав.

        """
        return True

    def event_get_todo(self):
        """ Получить можно только дело из своей записи """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)

    def event_update_todo(self):
        """ Обновить можно только дело из своей записи """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)

    def event_delete_todo(self):
        """ Удалить можно только дело из своей записи """
        note = self.event_data.get("note")
        return self._is_note_author(note=note)
